  ...
  return (
    <span className="relative inline-block [perspective:400px]">
      <span className="block transition-transform duration-500 ease-out [transform-style:preserve-3d] group-hover/nav:[transform:rotateX(180deg)]">
        ...
        <span className="absolute inset-0 block text-wine [backface-visibility:hidden] [transform:rotateX(180deg)]">
          {children}
        </span>
      </span>
    </span>
...
export default function Header() {
  ...
  return (
    <header className="pointer-events-none fixed inset-x-0 top-0 z-50 flex justify-center px-3 pt-4 sm:px-4">
      <div className="pointer-events-auto flex w-full max-w-[1200px] items-center justify-between gap-4 rounded-full border border-ink/10 bg-white/70 py-2.5 pl-6 pr-2.5 shadow-[0_8px_30px_rgba(0,0,0,0.12)] backdrop-blur-md">
        {/* Логотип */}
        <a href="/" className="font-display text-[22px] font-semibold leading-none tracking-[0.12em] text-ink">
          ÁLIS
        </a>

        {/* Навигация по центру */}
        <nav className="hidden items-center gap-5 text-[13.5px] tracking-wide text-ink/80 lg:flex xl:gap-6">
          {NAV.map((item) =>
            item.sub ? (
              <div key={item.label.ru} className="group relative">
                ...
                <div className="invisible absolute left-1/2 top-full w-56 -translate-x-1/2 pt-3 opacity-0 transition-all duration-200 group-hover:visible group-hover:opacity-100">
                  <div className="overflow-hidden rounded-2xl border border-cream/10 bg-ink/95 p-2 shadow-[0_12px_40px_rgba(0,0,0,0.5)] backdrop-blur-md">
                    {item.sub.map((s) => (
                      <a
                        key={s.label.ru}
                        href={s.href}
                        className="block rounded-xl px-4 py-2.5 text-[13.5px] text-cream/75 transition-colors hover:bg-cream/10 hover:text-cream"
                      >
                        {s.label[lang]}
                      </a>
                    ))}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <a
            href="/#online"
            className="flex items-center gap-2.5 rounded-full border border-wine bg-wine py-2 pl-5 pr-2 text-[13px] font-medium text-cream transition-colors duration-300 hover:bg-transparent hover:text-wine"
          >

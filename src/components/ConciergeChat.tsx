                ...
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-[13.5px] leading-relaxed ${m.from === "user" ? "bg-[#6E7248] text-[#f4efe6]" : "border border-[#17191a]/8 bg-white text-[#2a2320]"}`}>
                  ...
                  {m.actions?.map((a) => (
                    <a
                      ...
                      className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-[#6E7248] px-3.5 py-1.5 text-[12px] font-medium text-[#f4efe6] transition-transform hover:scale-[1.03]"
                    >
                      {a.label[lang]}
                    </a>
                  ))}
          ...
          <div className="flex flex-wrap gap-2 border-t border-[#17191a]/8 bg-white px-4 py-3">
            {TOPICS.map((tp) => (
              <button
                key={tp.chip.ru}
                onClick={() => ask(tp)}
                className="rounded-full border border-[#6E7248]/25 px-3 py-1.5 text-[12px] text-[#6E7248] transition-colors hover:bg-[#6E7248] hover:text-[#f4efe6]"
              >
                {tp.chip[lang]}
              </button>
            ))}

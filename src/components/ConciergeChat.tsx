    ...
    <div className="pointer-events-none fixed bottom-4 right-4 z-[120] flex flex-col items-end gap-3 sm:bottom-6 sm:right-6">
      {open && (
        <div className="pointer-events-auto flex h-[76vh] max-h-[600px] w-[calc(100vw-2rem)] max-w-[380px] flex-col overflow-hidden rounded-[24px] border border-[#17191a]/10 bg-white shadow-[0_24px_70px_rgba(0,0,0,0.28)]">
          ...
          <div className="flex items-center gap-3 bg-[#722F37] px-5 py-4 text-[#f4efe6]">
            <span className="relative shrink-0">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={ADMIN_PHOTO} alt="" className="h-11 w-11 rounded-full object-cover" />
              <span className="absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-[#722F37] bg-[#4ade80]" />
            </span>
          ...
          <div ref={bodyRef} className="flex-1 space-y-3 overflow-y-auto bg-[#faf7f2] px-4 py-4">
            {msgs.map((m, i) => (
              <div key={i} className={`flex ${m.from === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-2xl px-4 py-2.5 text-[13.5px] leading-relaxed ${m.from === "user" ? "bg-[#722F37] text-[#f4efe6]" : "border border-[#17191a]/8 bg-white text-[#2a2320]"}`}>
                  <p>{m.text}</p>
                  {m.actions?.map((a) => (
                    <a key={a.href} href={a.href} target="_blank" rel="noopener noreferrer" className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-[#722F37] px-3.5 py-1.5 text-[12px] font-medium text-[#f4efe6] transition-transform hover:scale-[1.03]">
                      {a.label[lang]}
                    </a>
                  ))}
                </div>
            ...
            {!formSent && (
              <form onSubmit={submitLead} className="rounded-2xl border border-[#722F37]/20 bg-white p-4 shadow-sm">
                <p className="text-[12px] font-medium uppercase tracking-[0.1em] text-[#722F37]">{t("Быстрая заявка", "Quick request")}</p>
                <input
                  value={name}
                  onChange={(e) => { setName(e.target.value); if (formErr) setFormErr(false); }}
                  placeholder={t("Ваше имя", "Your name")}
                  className={`mt-3 w-full rounded-xl border bg-[#faf7f2] px-4 py-2.5 text-[14px] text-[#2a2320] outline-none transition-colors placeholder:text-[#2a2320]/40 focus:border-[#722F37] ${formErr && !name.trim() ? "border-[#e7a0a0]" : "border-transparent"}`}
                />
                <input
                  inputMode="tel"
                  value={phone}
                  ...
                  className={`mt-2 w-full rounded-xl border bg-[#faf7f2] px-4 py-2.5 text-[14px] text-[#2a2320] outline-none transition-colors placeholder:text-[#2a2320]/40 focus:border-[#722F37] ${formErr && phone.replace(/\D/g, "").length < 11 ? "border-[#e7a0a0]" : "border-transparent"}`}
                />
                <button type="submit" className="mt-3 w-full rounded-xl bg-[#722F37] py-2.5 text-[13px] font-medium uppercase tracking-[0.1em] text-[#f4efe6] transition-colors hover:bg-[#5c2530]">
                  {t("Отправить", "Send")}
                </button>
              </form>
            )}
          ...
          <div className="flex flex-wrap gap-2 border-t border-[#17191a]/8 bg-white px-4 pt-3">
            {TOPICS.map((tp) => (
              <button key={tp.chip.ru} onClick={() => ask(tp)} className="rounded-full border border-[#722F37]/25 px-3 py-1.5 text-[12px] text-[#722F37] transition-colors hover:bg-[#722F37] hover:text-[#f4efe6]">
                {tp.chip[lang]}
              </button>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 border-t border-[#17191a]/8 bg-white px-3 py-3">
            ...
            />
            <button onClick={sendMsg} aria-label={t("Отправить", "Send")} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#722F37] text-[#f4efe6] transition-transform hover:scale-105">
      ...
      {open ? (
        <button
          ...
          className="pointer-events-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#722F37] text-[#f4efe6] shadow-[0_10px_30px_rgba(59,13,26,0.4)] transition-transform hover:scale-105"
        >
        ...
        <button
          ...
          className="pointer-events-auto flex items-center gap-2.5 rounded-full bg-[#722F37] py-3.5 pl-5 pr-6 text-[14px] font-medium text-[#f4efe6] shadow-[0_10px_30px_rgba(59,13,26,0.4)] transition-transform hover:scale-[1.03]"
        >
          <span className="relative flex h-6 w-6 shrink-0 items-center justify-center">
            ...
            <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border-2 border-[#722F37] bg-[#4ade80]" />
          </span>

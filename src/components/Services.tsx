...
function Tile({ s, i, started }: { s: Service; i: number; started: boolean }) {
  ...
  return (
    <a href="https://n1054895.yclients.com/company/976464/personal/menu" target="_blank" rel="noopener noreferrer" style={reveal} className="group relative block aspect-[4/5] overflow-hidden rounded-[22px] bg-[#f1ede6]">
      ...
      <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#722F37]/25 opacity-0 backdrop-blur-[6px] transition-opacity duration-500 ease-out group-hover:opacity-100">
    ...
    <section id="services" className="scroll-mt-24 bg-white py-24 lg:py-32">
      <div className="mx-auto w-[96%] max-w-[1620px]">
        ...
        <div className="mb-12 max-w-2xl lg:mb-16">
          <h2 className="font-display text-[34px] uppercase tracking-[0.06em] leading-[1.08] text-[#722F37] lg:text-[52px]">
            {en ? "Our services" : "Наши услуги"}
          </h2>
          <p className="mt-4 text-[15px] leading-relaxed text-[#17191a]/60 lg:text-[16px]">
            {en ? (
              <>Up-to-date prices are in the online booking. <span className="font-medium text-[#722F37]">−10% on your first visit.</span></>
            ) : (
              <>Актуальные цены — в онлайн-записи. <span className="font-medium text-[#722F37]">−10% на первое посещение.</span></>
            )}
          </p>
        </div>
        ...
        <div ref={gridRef} className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-3.5">
          ...
          <a
            ...
            className="group relative flex aspect-[4/5] flex-col justify-between overflow-hidden rounded-[22px] border-2 border-[#722F37] bg-[#722F37] p-5 text-[#ffffff] lg:p-6"
          >
            ...
            <span className="relative z-10 font-serif text-[21px] leading-tight transition-colors duration-500 group-hover:text-[#722F37] lg:text-[25px]">
              {en ? "Not sure what you need?" : "Не знаете, что выбрать?"}
            </span>
            <div className="relative z-10">
              <p className="mb-6 text-[13px] leading-relaxed text-[#ffffff]/75 transition-colors duration-500 group-hover:text-[#722F37]/80 lg:text-[14px]">
                {en
              ...
              <span className="inline-flex items-center gap-3.5 text-[13px] uppercase tracking-[0.14em] transition-colors duration-500 group-hover:text-[#722F37]">
                <span className="flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-full bg-[#ffffff] text-[#722F37] transition-colors duration-300 group-hover:bg-transparent">

        ...
        <div className="flex flex-col gap-3">
          {VACANCIES.map((v, i) => (
            <a
              key={v.role.ru}
              href="#join"
              className="group relative grid grid-cols-[auto_1fr_auto] items-center gap-5 rounded-[20px] border border-[#722F37]/12 px-6 py-6 transition-colors duration-300 hover:border-transparent hover:bg-[#722F37] lg:gap-8 lg:px-8 lg:py-8"
            >
              {/* Номер */}
              <span className="font-display text-[13px] tabular-nums text-[#722F37] transition-colors duration-300 group-hover:text-[#f4efe6]/70 lg:text-[15px]">
                {String(i + 1).padStart(2, "0")}
              </span>

              {/* Название + описание */}
              <div className="min-w-0">
                <h3 className="font-display text-[16px] font-normal uppercase leading-[1.15] tracking-[0.01em] text-[#722F37] transition-all duration-300 group-hover:translate-x-2 group-hover:text-[#f4efe6] sm:text-[19px] lg:text-[24px]">
                  {v.role[lang]}
                </h3>
              ...
              <span className="flex items-center gap-4 lg:gap-6">
                <span className="hidden whitespace-nowrap rounded-full bg-[#722F37]/10 px-3 py-1 text-[11px] uppercase tracking-[0.12em] text-[#722F37] transition-colors duration-300 group-hover:bg-[#f4efe6]/15 group-hover:text-[#f4efe6] sm:inline">
                  {v.schedule[lang]}
                </span>
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-[#722F37]/30 text-[#722F37] transition-all duration-300 group-hover:border-[#f4efe6] group-hover:bg-[#f4efe6] group-hover:text-[#722F37] lg:h-12 lg:w-12">

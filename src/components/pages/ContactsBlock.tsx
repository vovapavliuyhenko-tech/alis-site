...
export default function ContactsBlock() {
  ...
  return (
    <section className="bg-white px-3 pt-[76px] pb-16 sm:px-4 lg:pt-[88px] lg:pb-24">
      ...
      <div className="overflow-hidden rounded-[30px] border border-[#722F37]/15 bg-white lg:grid lg:grid-cols-2">
        {/* Левая колонка */}
        <div className="flex flex-col p-8 lg:p-12">
          <p className="flex items-center gap-2 text-[11px] uppercase tracking-[0.22em] text-[#722F37]">
          ...
          <h2 className="mt-5 font-display text-[28px] font-normal uppercase leading-[1.05] tracking-[0.01em] text-[#722F37] sm:text-[36px] lg:text-[46px]">
          ...
          <dl className="mt-8 flex flex-col gap-6 lg:mt-10">
            {INFO.map((r) => (
              <div key={r.label.ru} className="grid grid-cols-[110px_1fr] items-baseline gap-5 lg:grid-cols-[150px_1fr]">
                ...
                <dd className={`text-[15px] font-medium leading-snug lg:text-[16px] ${r.accent ? "text-[#722F37]" : "text-[#2a2320]"}`}>
                  {r.href ? (
                    <a href={r.href} target={r.href.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer" className="transition-colors hover:text-[#722F37]">
                      {r.value}
                    </a>
                  ) : (
                    r.value
          ...
          <div className="mt-10 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {BTNS.map((b, i) => (
              ...
              >
                <span className="max-w-[80%] font-display text-[13px] uppercase leading-[1.3] tracking-[0.06em] text-[#722F37] lg:text-[14px]">
                  {b.label[lang]}
                </span>
                {b.handle && <span className="mt-1 text-[12px] text-[#722F37]/60">{b.handle}</span>}
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.2"
                  className="absolute bottom-5 right-5 h-6 w-6 text-[#722F37] transition-transform duration-300 group-hover:translate-x-0.5 group-hover:translate-y-0.5"
                >
        ...
        <div className="relative min-h-[360px] lg:min-h-[560px]">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="absolute bottom-4 left-4 inline-flex items-center gap-2 rounded-full bg-white/95 px-4 py-2.5 text-[12px] font-medium text-[#722F37] shadow-[0_8px_24px_rgba(0,0,0,0.14)] backdrop-blur-sm transition-colors hover:bg-white"
          >
            <span className="text-[#722F37]">{IconPin}</span>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="absolute bottom-4 right-4 inline-flex items-center justify-center rounded-full bg-[#722F37] px-6 py-3.5 font-display text-[12px] uppercase tracking-[0.14em] text-[#f4efe6] shadow-[0_10px_30px_rgba(114,47,55,0.4)] transition-colors hover:bg-[#5c2530] lg:px-8 lg:py-4 lg:text-[13px]"
          >

      ...
      <div className="mx-auto w-[92%] max-w-[1200px]">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:gap-8">
          {categories.map((c, i) => (
            <article
              key={c.label.ru}
              className="group overflow-hidden rounded-[24px] border border-[#722F37]/12 bg-white shadow-[0_16px_44px_rgba(0,0,0,0.06)]"
            >
              ...
              <div className="px-6 py-5 lg:px-8 lg:py-6">
                {c.rows.map((r) => (
                  <div key={r.name.ru} className="flex items-baseline gap-4 border-t border-[#722F37]/10 py-3 first:border-t-0">
                    <span className="min-w-0 text-[14px] text-[#2a2320] lg:text-[15px]">{r.name[lang]}</span>
                    <span className="mx-1 flex-1 translate-y-[-3px] border-b border-dotted border-[#722F37]/20" />
                    <span className="whitespace-nowrap font-display text-[14px] text-[#722F37] lg:text-[16px]">{r.price[lang]}</span>
                  </div>
                ))}
              </div>
            </article>

      ...
      <div className={`mx-auto grid w-[92%] max-w-[1180px] items-center gap-8 lg:grid-cols-2 lg:gap-14 ${mirror ? "" : ""}`}>
        ...
        <div className={`relative ${mirror ? "lg:order-2" : "lg:order-1"}`}>
          <div className="relative aspect-[4/5] w-full overflow-hidden rounded-[24px] lg:aspect-[5/6]">
            ...
            {tag && (
              <span className="absolute bottom-4 left-4 rounded-full bg-[#f4efe6]/90 px-4 py-2 text-[11px] uppercase tracking-[0.14em] text-[#722F37] backdrop-blur-sm">
                {tag[lang]}
              </span>
            )}
          </div>
        ...
        <div className={mirror ? "lg:order-1" : "lg:order-2"}>
          <span className="text-[11px] uppercase tracking-[0.22em] text-[#722F37]">{eyebrow[lang]}</span>
          <h2 className="mt-4 font-display text-[24px] uppercase tracking-[0.05em] leading-[1.1] text-[#722F37] lg:text-[36px]">
            {title[lang]}
          </h2>
          ...
          {ctaLabel && ctaHref && (
            <a
              ...
              className="group/btn mt-8 inline-flex items-center gap-3 rounded-full bg-[#722F37] px-8 py-4 text-[13px] font-medium uppercase tracking-[0.1em] text-[#f4efe6] transition-transform duration-300 hover:scale-[1.02]"
            >
              {ctaLabel[lang]}

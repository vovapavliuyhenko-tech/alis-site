  ...
  return (
    <section className={ground === "cream" ? "bg-[#f7f3ed] py-24 lg:py-28" : "bg-white py-24 lg:py-28"}>
      <div className="mx-auto w-[92%] max-w-[1100px]">
            ...
            return (
              <button
                ...
                className={`relative pb-2 font-display text-[15px] uppercase tracking-[0.08em] transition-colors duration-200 lg:text-[17px] ${
                  on ? "text-[#3B0D1A]" : "text-[#2a2320]/45 hover:text-[#2a2320]/80"
                }`}
              >
                {c.label[lang]}
                <span
                  className={`absolute inset-x-0 bottom-0 h-px origin-center bg-[#3B0D1A] transition-transform duration-300 ${
        ...
        >
          {rows.map((r) => (
            <div key={r.name.ru} className="flex items-baseline gap-4 border-t border-[#3B0D1A]/12 py-4 last:border-b">
              ...
              <span className="whitespace-nowrap font-display text-[15px] text-[#3B0D1A] lg:text-[17px]">
                {r.price[lang]}
              </span>
            </div>
          ))}

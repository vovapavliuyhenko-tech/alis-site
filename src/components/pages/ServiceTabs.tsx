    ...
    <section className={ground === "cream" ? "bg-[#f7f3ed] py-24 lg:py-28" : "bg-white py-24 lg:py-28"}>
      <div className="mx-auto w-[92%] max-w-[1100px]">
        <div className="mb-10 text-center lg:mb-14">
          ...
          <h2 className="mx-auto mt-5 max-w-2xl font-display text-[26px] font-normal uppercase leading-[1.1] tracking-[0.03em] text-[#46131E] lg:text-[40px]">
            {title[lang]}
          </h2>
        </div>
            ...
            return (
              <button
                ...
                className={`relative pb-2 font-display text-[15px] uppercase tracking-[0.08em] transition-colors duration-200 lg:text-[17px] ${
                  on ? "text-[#46131E]" : "text-[#2a2320]/45 hover:text-[#2a2320]/80"
                }`}
              >
                {c.label[lang]}
                <span
                  className={`absolute inset-x-0 bottom-0 h-px origin-center bg-[#46131E] transition-transform duration-300 ${
        ...
        >
          {rows.map((r) => (
            <div key={r.name.ru} className="flex items-baseline gap-4 border-t border-[#46131E]/12 py-4 last:border-b">
              ...
              {/* пунктирный лидер */}
              <span className="mx-1 flex-1 translate-y-[-3px] border-b border-dotted border-[#46131E]/25" />
              <span className="whitespace-nowrap font-display text-[15px] text-[#46131E] lg:text-[17px]">
                {r.price[lang]}
              </span>
            </div>
          ))}

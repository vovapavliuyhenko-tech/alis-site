...
export default function ConsultForm() {
  ...
  const inputCls =
    "w-full rounded-xl border border-[#17191a]/15 bg-white px-4 py-3.5 text-[14px] text-[#17191a] outline-none transition-colors placeholder:text-[#17191a]/40 focus:border-[#46131E]";
  ...
  return (
    <section id="request" className="scroll-mt-24 bg-white py-24 lg:py-28">
      <div className="mx-auto w-[92%] max-w-[1200px]">
        <div className="grid overflow-hidden rounded-[32px] bg-[#46131E] text-[#f4efe6] lg:grid-cols-2">
          {/* Левая часть — оффер */}
          <div className="relative flex flex-col justify-center px-7 py-12 sm:px-10 lg:px-14 lg:py-16">
            <span aria-hidden className="pointer-events-none absolute -left-20 -top-20 h-64 w-64 rounded-full bg-[#46131E]/25 blur-[90px]" />
              ...
              <ul className="mt-8 space-y-3 text-[14px] text-[#f4efe6]/85">
                {[
                  ...
                  <li key={b} className="flex items-start gap-3">
                    <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#46131E]" />
                    {b}
                  </li>
                ))}
              </ul>
          ...
          <div className="bg-[#f4efe6] p-6 sm:p-9 lg:p-11">
            {sent ? (
              <div className="flex h-full min-h-[320px] flex-col items-center justify-center text-center text-[#17191a]">
                <span className="flex h-16 w-16 items-center justify-center rounded-full bg-[#46131E] text-[#f4efe6]">
                ...
                <p className="mt-6 font-display text-[24px] uppercase tracking-[0.04em] text-[#46131E]">{t("Заявка отправлена", "Request sent")}</p>
              ...
              <form onSubmit={submit} className="space-y-5">
                ...
                <label className="flex items-start gap-3 text-[12.5px] leading-relaxed text-[#17191a]/60">
                  <input type="checkbox" checked={agree} onChange={(e) => { setAgree(e.target.checked); setErr(false); }} className="mt-0.5 h-4 w-4 shrink-0 accent-[#46131E]" />
                  <span>
                    {t("Согласен на обработку данных и ", "I agree to data processing and the ")}
                    <a href="/policy" className="text-[#46131E] underline underline-offset-2">{t("политику конфиденциальности", "privacy policy")}</a>
                  </span>
                </label>

                {err && (
                  <p className="text-[12.5px] text-[#46131E]">{t("Заполните имя, телефон и согласие.", "Please fill in name, phone and consent.")}</p>
                )}

                <button
                  type="submit"
                  className="group flex w-full items-center justify-center gap-3 rounded-xl bg-[#46131E] py-4 font-display text-[13px] uppercase tracking-[0.18em] text-[#f4efe6] transition-colors duration-300 hover:bg-[#46131E]"
                >

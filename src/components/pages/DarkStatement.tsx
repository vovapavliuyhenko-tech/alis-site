...
export default function DarkStatement({
  ...
  const { lang } = useLang();
  const accentColor = accent === "gold" ? "text-[#8a5a3c]" : "text-[#9ea05f]";
  return (
    <section className="scroll-mt-24 bg-white py-14 lg:py-20">
      <div className="mx-auto w-[92%] max-w-[1180px]">
        <div className="relative overflow-hidden rounded-[32px] bg-[#6E7248] px-7 py-16 text-[#f4efe6] sm:px-10 lg:px-16 lg:py-24">
          <span aria-hidden className="pointer-events-none absolute -left-24 -top-24 h-72 w-72 rounded-full bg-[#8a5a3c]/10 blur-[90px]" />
          <span aria-hidden className="pointer-events-none absolute -bottom-24 right-1/4 h-72 w-72 rounded-full bg-[#6E7248]/25 blur-[90px]" />
          <div className="relative max-w-3xl">
            <span className="text-[11px] uppercase tracking-[0.22em] text-[#8a5a3c]">{eyebrow[lang]}</span>
            ...
            {ctaLabel && ctaHref && (
              <a
                ...
                className="group/btn mt-9 inline-flex items-center gap-3 rounded-full bg-[#f4efe6] px-8 py-4 text-[14px] font-medium uppercase tracking-[0.1em] text-[#6E7248] transition-transform duration-300 hover:scale-[1.02]"
              >
                {ctaLabel[lang]}

  ...
  return (
    <section className="scroll-mt-24 bg-white py-16 lg:py-24">
      <div className="mx-auto w-[92%] max-w-[1180px]">
        <div className={`overflow-hidden rounded-[28px] px-7 py-12 sm:px-10 lg:px-14 lg:py-16 ${dark ? "bg-[#722F37] text-[#f4efe6]" : "border border-[#17191a]/10 bg-[#faf7f2] text-[#2a2320]"}`}>
          <span className={`inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-[11px] uppercase tracking-[0.2em] ${dark ? "bg-[#722F37]/25 text-[#f4efe6]" : "bg-[#722F37]/12 text-[#722F37]"}`}>
            {eyebrow[lang]}
          </span>
          <h2 className={`mt-5 max-w-2xl font-display text-[24px] uppercase tracking-[0.06em] leading-[1.12] lg:text-[36px] ${dark ? "" : "text-[#722F37]"}`}>
            {title[lang]}
          </h2>
          {text && (
          ...
          {ctaLabel && ctaHref && (
            <a
              ...
              className={`group/btn mt-8 inline-flex items-center gap-3 rounded-full px-8 py-4 text-[14px] font-medium uppercase tracking-[0.1em] transition-transform duration-300 hover:scale-[1.02] ${dark ? "bg-[#f4efe6] text-[#722F37]" : "bg-[#722F37] text-[#f4efe6]"}`}
            >
              {ctaLabel[lang]}

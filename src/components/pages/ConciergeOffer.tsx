  ...
  return (
    <section id="offer" className="scroll-mt-24 bg-white px-3 py-16 sm:px-4 lg:py-24">
      <div className="mx-auto grid w-full max-w-[1280px] grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-2">
        {/* Левая колонка — оффер */}
        <div className="flex flex-col justify-center rounded-[28px] border border-[#591C28]/15 bg-[#F9F8F6] p-8 lg:p-12">
          <p className="text-[11px] uppercase tracking-[0.24em] text-[#591C28]">{t("коммерческое предложение", "our proposal")}</p>
          <h2 className="mt-5 font-display text-[22px] font-normal uppercase leading-[1.2] tracking-[0.02em] text-[#591C28] lg:text-[28px]">
          ...
          <p className="mt-9 text-[10px] uppercase tracking-[0.2em] text-[#591C28]/50">{t("что входит", "what's included")}</p>
          <ul className="mt-2 flex flex-col">
            {INCLUDED.map((it, i) => (
              <li key={it.ru} className={`flex items-center gap-4 py-4 ${i > 0 ? "border-t border-[#591C28]/12" : ""}`}>
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#591C28] text-[#f4efe6]">
        ...
        <div id="booking" className="scroll-mt-24 flex flex-col rounded-[28px] bg-[#591C28] px-6 py-12 text-[#f4efe6] sm:px-10 lg:px-14 lg:py-16">
          {sent ? (
            ...
            <form onSubmit={submit} noValidate className="flex flex-1 flex-col justify-between gap-8">
              ...
              <button type="submit" className="mx-auto flex w-full max-w-lg items-center justify-center rounded-2xl border border-[#f4efe6] bg-[#f4efe6] py-4 font-display text-[13px] uppercase tracking-[0.16em] text-[#591C28] transition-colors duration-300 hover:bg-transparent hover:text-[#f4efe6] sm:text-[14px]">
                {t("Оставить заявку", "Leave a request")}
              </button>
            </form>
          )}

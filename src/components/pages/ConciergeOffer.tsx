          ...
          {sent ? (
            ...
            <form onSubmit={submit} noValidate className="flex flex-1 flex-col justify-between gap-8">
              ...
              <div className="mx-auto flex w-full max-w-lg flex-col gap-3">
                {FIELDS.map((f) => (
                  <label key={f.key} className={`block rounded-2xl border bg-white px-5 py-3 transition-colors focus-within:border-[#8a5a3c] ${errors[f.key] ? "border-[#e7a0a0]" : "border-transparent"}`}>
                    <span className="mb-1 block text-[10px] uppercase tracking-[0.16em] text-[#46131E]/60">
                      {f.label[lang]}
                      {f.required && <span className="text-[#8a5a3c]"> *</span>}
                    </span>
                    {f.textarea ? (
                      <textarea rows={2} value={values[f.key] || ""} onChange={(e) => set(f.key, e.target.value)} className="w-full resize-none bg-transparent text-[15px] text-[#2a2320] outline-none placeholder:text-[#2a2320]/35" />
                    ) : (
                      <input
                        type={f.type || "text"}

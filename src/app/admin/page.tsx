      ...
      <main className="flex min-h-svh items-center justify-center bg-white px-6">
        <div className="w-full max-w-sm">
          ...
          <input
            ...
            placeholder="Пароль"
            className="w-full border-b border-[#17191a]/25 bg-transparent pb-3 text-[16px] text-[#17191a] outline-none focus:border-[#722F37]"
          />
          <button
            onClick={() => {
            ...
            className="mt-6 w-full rounded-full border border-[#722F37] bg-[#722F37] py-3 text-[14px] font-medium text-[#f4efe6] transition-colors hover:bg-transparent hover:text-[#722F37]"
          >
            Войти
          </button>
          <p className="mt-4 text-[12px] text-[#17191a]/35">Пароль прототипа: <code>alis</code></p>
          <a href="/" className="mt-6 block text-[13px] text-[#722F37] underline underline-offset-4">← на сайт</a>
        </div>
      </main>
    );
  }
      ...
      <div className="mx-auto w-[94%] max-w-[1100px] py-10">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 text-[13px]">
            <a href="/#online" className="text-[#722F37] underline underline-offset-4">Открыть витрину</a>
            <button
            ...
            <button
              ...
              className="rounded-full border border-[#17191a]/20 px-4 py-1.5 hover:border-[#722F37]"
            >
              Выйти
            </button>
          </div>
          ...
          {([["services", "Услуги и цены"], ["masters", "Мастера и график"], ["bookings", `Записи (${store.bookings.length})`]] as [Tab, string][]).map(([k, label]) => (
            <button
              ...
              className={`-mb-px border-b-2 px-4 py-2.5 text-[14px] transition-colors ${
                tab === k ? "border-[#722F37] text-[#722F37]" : "border-transparent text-[#17191a]/50 hover:text-[#17191a]"
              }`}
            >
              {label}
            </button>
...
function ServicesTab({ store, commit }: { store: Store; commit: (s: Store) => void }) {
  ...
  return (
    <div>
            ...
            {store.services.map((s) => (
              <tr key={s.id} className="border-t border-[#17191a]/10">
                <td className="py-3 pr-4">
                  <input value={s.title} onChange={(e) => update(s.id, { title: e.target.value })} className="w-full rounded-lg border border-[#17191a]/15 px-3 py-2 outline-none focus:border-[#722F37]" />
                </td>
                <td className="py-3 pr-4">
                  <input type="number" value={s.price} onChange={(e) => update(s.id, { price: Number(e.target.value) })} className="w-28 rounded-lg border border-[#17191a]/15 px-3 py-2 tabular-nums outline-none focus:border-[#722F37]" />
                </td>
                <td className="py-3 pr-4">
                  <input type="number" step={5} value={s.durationMin} onChange={(e) => update(s.id, { durationMin: Number(e.target.value) })} className="w-24 rounded-lg border border-[#17191a]/15 px-3 py-2 tabular-nums outline-none focus:border-[#722F37]" />
                </td>
                <td className="py-3 text-right">
                  <button onClick={() => del(s.id)} className="text-[13px] text-[#722F37]/70 hover:text-[#722F37]">удалить</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <button onClick={add} className="mt-6 rounded-full border border-[#722F37] px-5 py-2.5 text-[13px] text-[#722F37] transition-colors hover:bg-[#722F37] hover:text-[#f4efe6]">
        + услуга
      </button>
    </div>
  );
...
function MastersTab({ store, commit }: { store: Store; commit: (s: Store) => void }) {
  ...
  return (
    <div className="space-y-6">
      {store.masters.map((m) => (
        <div key={m.id} className="rounded-2xl border border-[#17191a]/10 p-5">
          <div className="mb-5 flex flex-wrap items-center gap-4">
            <input value={m.name} onChange={(e) => update(m.id, { name: e.target.value })} className="rounded-lg border border-[#17191a]/15 px-3 py-2 text-[15px] outline-none focus:border-[#722F37]" />
            <input value={m.role} onChange={(e) => update(m.id, { role: e.target.value })} className="rounded-lg border border-[#17191a]/15 px-3 py-2 text-[13px] outline-none focus:border-[#722F37]" />
            <label className="flex items-center gap-2 text-[13px] text-[#17191a]/60">
              шаг слота
              <input type="number" step={5} value={m.stepMin} onChange={(e) => update(m.id, { stepMin: Number(e.target.value) })} className="w-20 rounded-lg border border-[#17191a]/15 px-3 py-2 tabular-nums outline-none focus:border-[#722F37]" />
              мин
            </label>
            <button onClick={() => del(m.id)} className="ml-auto text-[13px] text-[#722F37]/70 hover:text-[#722F37]">удалить мастера</button>
          </div>
            ...
            {store.services.map((s) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full border px-3.5 py-1.5 text-[13px] transition-colors ${on ? "border-[#722F37] bg-[#722F37] text-[#f4efe6]" : "border-[#17191a]/15 text-[#17191a]/70 hover:border-[#722F37]/50"}`}
                >
                  {s.title}
                </button>
              );
            ...
            {m.week.map((d, i) => (
              <div key={i} className="flex items-center gap-2 rounded-lg border border-[#17191a]/10 px-3 py-2">
                <button
                  ...
                  className={`flex h-6 w-12 shrink-0 items-center rounded-full px-0.5 transition-colors ${d.on ? "bg-[#722F37]" : "bg-[#17191a]/15"}`}
      ...
      <button onClick={add} className="rounded-full border border-[#722F37] px-5 py-2.5 text-[13px] text-[#722F37] transition-colors hover:bg-[#722F37] hover:text-[#f4efe6]">
        + мастер
      </button>
    </div>
  );
...
function TimeInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <input type="time" value={value} onChange={(e) => onChange(e.target.value)} className="rounded-lg border border-[#17191a]/15 px-2 py-1 text-[13px] tabular-nums outline-none focus:border-[#722F37]" />
  );
}
          ...
          {rows.map((b) => {
            ...
            return (
              <tr key={b.id} className={`border-t border-[#17191a]/10 ${upcoming ? "" : "text-[#17191a]/40"}`}>
                ...
                <td className="py-3 text-right"><button onClick={() => del(b.id)} className="text-[13px] text-[#722F37]/70 hover:text-[#722F37]">отменить</button></td>
              </tr>
            );
          })}
        </tbody>
